...
namespace Git_Support_Tool
{
    public partial class Form1 : Form
    {
        ...
        private void btn_AddFiles_Click(object sender, EventArgs e)
        {
            ...
            if (ret == DialogResult.OK && openFileDialog.FileNames.Length != 0)
            {
                foreach (var item in openFileDialog.FileNames)
                {
                    this.rtb_Result.Text += DateTime.Now.ToLongTimeString() + ": ";
                    this.rtb_Result.Text += myclass.Do(" add " + '"' + item + '"', this.txt_FolderProject.Text);
                    this.rtb_Result.Text += "Added file " + item + "\n------------------------------------------------------------\n";
                }
            }
        }
